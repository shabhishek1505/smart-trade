...
export default function SettingsPanel({ stocks, onSave, onClose, onResultsRefresh }) {
  ...
  const handleRunNow = async () => {
    ...
    try {
      ...
      if (data.error) { setRunState("error"); setRunMsg(data.error); return; }
  ...
  const runBtnLabel = { idle: "Run Now", triggered: "Triggering…", running: "Running…", done: "Done ✓", error: "Retry" }[runState];
  const runMsgColor = runState === "error" ? "text-red-500" : runState === "done" ? "text-green-600 dark:text-green-400" : "text-blue-600 dark:text-blue-400";
  ...
  const cardCls  = "bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm border border-gray-100 dark:border-gray-700";
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        <div className={`${cardCls} mb-5`}>
          <h2 className="font-semibold text-gray-800 dark:text-gray-200 mb-3">Manual Analysis</h2>
          <button
          ...
          >
            {(runState === "triggered" || runState === "running") ? <Loader size={16} className="animate-spin" /> : <PlayCircle size={16} />}
            {runBtnLabel}
          </button>
        ...
        <form onSubmit={handleSave} className="space-y-5">
          ...
          <div className={`${cardCls} space-y-4`}>
            <h2 className="font-semibold text-gray-800 dark:text-gray-200">Telegram</h2>
            <div>
              <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1">Group Chat ID</label>
              <input className={inputCls} placeholder="-1001234567890" value={form.telegram_group_chat_id} onChange={(e) => toggle("telegram_group_chat_id", e.target.value)} />
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 flex items-start gap-1">
                <HelpCircle size={12} className="mt-0.5 shrink-0" />
                Negative number. Add bot to group → send a message → visit getUpdates URL
              </p>
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1">Private Chat ID</label>
              <input className={inputCls} placeholder="123456789" value={form.telegram_private_chat_id} onChange={(e) => toggle("telegram_private_chat_id", e.target.value)} />
            </div>
          </div>
          ...
          <div className={`${cardCls} space-y-4`}>
            ...
            <div>
              <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1">Run Time (UTC)</label>
              <input className={inputCls} placeholder="10:17" value={form.run_time_utc} onChange={(e) => toggle("run_time_utc", e.target.value)} />
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">= {istTime}</p>
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-2">Notify on</label>
