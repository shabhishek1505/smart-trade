...
def run(df, config):
    ...
    indicators = {
        ...
        "EMA 20": round(ema20_val, 2),
        "EMA 50": round(ema50_val, 2),
        "Price vs EMA20": f"{'+' if close_val >= ema20_val else ''}{close_val - ema20_val:.2f}",
        "Price vs EMA50": f"{'+' if close_val >= ema50_val else ''}{close_val - ema50_val:.2f}",
    }
    ...
    if rsi_val > 70 or close_val < ema50_val:
        ...
        return {**_avoid(reason), "indicators": indicators}

    return {**_neutral(f"RSI {rsi_val:.1f} — no clear signal"), "indicators": indicators}
